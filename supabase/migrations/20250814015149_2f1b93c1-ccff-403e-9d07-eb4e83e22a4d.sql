...
-- Add new columns to weeks table
ALTER TABLE public.weeks 
ADD COLUMN prerequisites JSONB DEFAULT '[]'::jsonb,
ADD COLUMN difficulty_level difficulty_level DEFAULT 'beginner',
ADD COLUMN estimated_duration INTEGER DEFAULT 60, -- minutes
ADD COLUMN points_reward INTEGER DEFAULT 100;

-- Create week_progress table for comprehensive progress tracking
CREATE TABLE public.week_progress (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL,
  week_id UUID NOT NULL REFERENCES public.weeks(id) ON DELETE CASCADE,
  progress_percentage INTEGER DEFAULT 0 CHECK (progress_percentage >= 0 AND progress_percentage <= 100),
  current_step TEXT, -- tracks where user left off (module_id, task_id, etc.)
  ...
  review_steps_completed INTEGER DEFAULT 0,
...
-- Create RLS policies for week_progress
...
CREATE POLICY "Admins can view all week progress" 
ON public.week_progress 
FOR SELECT 
USING (has_role(auth.uid(), 'admin'::app_role));

-- Create function to calculate week progress percentage
...
-- Create function to update week progress
...
-- Create trigger function to auto-update week progress
...
-- Create triggers to auto-update week progress
CREATE TRIGGER update_week_progress_on_module_completion
AFTER INSERT OR UPDATE ON public.module_progress
FOR EACH ROW EXECUTE FUNCTION public.auto_update_week_progress();

CREATE TRIGGER update_week_progress_on_submission
AFTER INSERT ON public.submissions
FOR EACH ROW EXECUTE FUNCTION public.auto_update_week_progress();

CREATE TRIGGER update_week_progress_on_step_completion
AFTER INSERT OR UPDATE ON public.step_progress
FOR EACH ROW EXECUTE FUNCTION public.auto_update_week_progress();

-- Create function to check week prerequisites
...
-- Update sample week data with new fields
UPDATE public.weeks 
SET 
...
-- Add prerequisites example for week 2 (requires week 1)
UPDATE public.weeks 
SET 
  ...
  points_reward = 200,
  prerequisites = (
    SELECT jsonb_build_array(id::text) 
    FROM public.weeks 
    WHERE week_number = 1
  )
WHERE week_number = 2;